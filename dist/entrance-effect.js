...
(() => {
  ...
  let paused = reduced.matches, frame = 0, width, height;
  const image = new Image();
  image.onload = () => { art.style.backgroundImage = 'url("assets/form.webp")'; art.classList.add('loaded'); };
  image.src = 'assets/form.webp';
  function resize() { const dpr = Math.min(devicePixelRatio || 1, 2); width = innerWidth; height = root.clientHeight; canvas.width = width*dpr; canvas.height = height*dpr; ctx.setTransform(dpr,0,0,dpr,0,0); draw(0); }
  function draw(time) {
  ...
  function loop(time) { draw(time); if(!paused&&!document.hidden) frame=requestAnimationFrame(loop); }
